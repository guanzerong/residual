...
class PreTrainedConfig(draccus.ChoiceRegistry, HubMixin, abc.ABC):
    ...
    def from_pretrained(
        ...
        **policy_kwargs,
        ...
        # First load the config file to filter out the 'type' field if it exists
        # since 'type' is a property, not a dataclass field that draccus can handle
        import json

        if config_file:
            with open(config_file) as f:
                config_dict = json.load(f)
            # Remove 'type' field if it exists since it's a computed property
            ...
            with tempfile.NamedTemporaryFile(mode="w", suffix=".json", delete=False) as temp_f:
                json.dump(config_dict, temp_f, indent=4)
                temp_config_file = temp_f.name

            try:
                result = draccus.parse(cls, temp_config_file, args=cli_overrides)
            finally:
