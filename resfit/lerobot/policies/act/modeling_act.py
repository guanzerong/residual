...
class ACTPolicy(PreTrainedPolicy):
    ...
    def reset(self, env_ids: list[int] | None = None):
        """Reset internal per-environment caches.

        Args:
            env_ids: Optional list of environment indices to reset. If ``None`` (default) all
                environments are reset. This should be called by the environment wrapper
                whenever an episode terminates for the corresponding environment index.
        """
        # Lazily-initialised containers might not exist yet (e.g. before the first call
        # to :py:meth:`select_action`). Guard against that here.
        if self.config.temporal_ensemble_coeff is not None:
            # --- Temporal ensembling branch -----------------------------------------------------
            if not hasattr(self, "_temporal_ensemblers"):
                # The list will be initialised on the first forward pass- nothing to reset yet.
                return
            if env_ids is None:
                for ensembler in self._temporal_ensemblers:
                    ensembler.reset()
            else:
                for idx in env_ids:
                    if idx < len(self._temporal_ensemblers):
                        self._temporal_ensemblers[idx].reset()
        else:
            # --- Action-queue branch -------------------------------------------------------------
            if not hasattr(self, "_action_queues"):
                # Will be lazily initialised later- nothing to reset yet.
                return
            if env_ids is None:
                # Clear the queues for *all* environments.
    ...
    def _ensure_action_queues(self, batch_size: int):
        """Create/resize per-environment deques lazily when we first see a batch"""

        if not hasattr(self, "_action_queues"):
            self._action_queues = []
        # Grow the list if we encounter more environments than seen previously.
        while len(self._action_queues) < batch_size:
            self._action_queues.append(deque([], maxlen=self.config.n_action_steps))

        # Shrink if batch size decreased (can happen with non-blocking VecEnvs).
        if len(self._action_queues) > batch_size:
            self._action_queues = self._action_queues[:batch_size]

    def _ensure_temporal_ensemblers(self, batch_size: int):
        """Lazy initialisation for per-environment temporal ensemblers."""
        if not hasattr(self, "_temporal_ensemblers"):
            self._temporal_ensemblers = []
        while len(self._temporal_ensemblers) < batch_size:
            self._temporal_ensemblers.append(
                ACTTemporalEnsembler(self.config.temporal_ensemble_coeff, self.config.chunk_size)
            )
    ...
    def encode_observation_features(self, batch: dict[str, Tensor]) -> Tensor:
        """Return frozen inference-time ACT encoder tokens as ``[B, S, D]``."""
        self.eval()
        batch, _ = self._prepare_action_inference_batch(batch)
        return self.model.encode_observation_features(batch)
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        # ------------------------------------------------------------------
        if self.config.temporal_ensemble_coeff is not None:
            # Ensure we have an ensembler per environment.
            self._ensure_temporal_ensemblers(batch_size)

            # Forward pass once for all environments.
            actions_seq = self.model(batch)[0]  # (B, chunk_size, action_dim)
            actions_seq = self.unnormalize_outputs({"action": actions_seq})["action"]

            ensembled_actions = []
            for idx in range(batch_size):
                # Each ensembler expects a (1, chunk_size, action_dim) tensor.
                action_i = self._temporal_ensemblers[idx].update(actions_seq[idx : idx + 1])
                ensembled_actions.append(action_i)
            return torch.cat(ensembled_actions, dim=0)

        # ------------------------------------------------------------
        # Case 2: standard action-chunking with per-env action queues.
        # ------------------------------------------------------------
        # Lazily create a deque per environment.
        self._ensure_action_queues(batch_size)

        # Identify environments that require a new action chunk (empty queue).
        ...
        if envs_needing_chunk:
            ...
            for idx in envs_needing_chunk:
                # Convert (n_steps, act_dim) tensor to a sequence of tensors and populate the deque.
                self._action_queues[idx].extend(actions_seq[idx].unbind(0))

        # Pop the next action for every environment and stack them back into a batch tensor.
        actions_to_execute = [q.popleft() for q in self._action_queues]
...
class ACT(nn.Module):
    ...
    def encode_observation_features(self, batch: dict[str, Tensor]) -> Tensor:
        """Run the inference-time transformer encoder and return tokens as ``[B, S, D]``.

        The VAE latent is set to zeros, matching the action-selection path when
        no target action sequence is provided.
        """
        if "observation.images" in batch:
            batch_size = batch["observation.images"][0].shape[0]
        ...
        encoder_in_pos_embed = torch.stack(encoder_in_pos_embed, axis=0)
        encoder_out = self.encoder(encoder_in_tokens, pos_embed=encoder_in_pos_embed)
        return encoder_out.transpose(0, 1)
