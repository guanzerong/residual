...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--label-point-count", type=int, default=10)
...
def _build_point_patch_infos(
    ...
    patch_rows, patch_cols = patch_hw
    point_indices = _sample_point_indices(len(pixels_rc), sample_count)
    infos: list[dict[str, int]] = []
    for order_idx, point_idx in enumerate(point_indices, start=1):
        row_px, col_px = pixels_rc[point_idx]
        patch_row, patch_col = _pixel_to_patch_coord(
            row_px=row_px,
            col_px=col_px,
            image_hw=image_hw,
            proc_hw=proc_hw,
        ...
        infos.append(
            {
                "label": order_idx,
                "trajectory_point_index": int(point_idx),
                "rgb_row": int(round(float(row_px))),
                "rgb_col": int(round(float(col_px))),
                "patch_row": int(patch_row),
                "patch_col": int(patch_col),
                "patch_linear_index": int(patch_row * patch_cols + patch_col),
            }
        )
    return infos
...
def _annotate_point_patch_infos(
    ax,
    point_patch_infos: list[dict[str, int]],
    *,
    ...
    src_h, src_w = src_hw
    dst_h, dst_w = dst_hw
    row_scale = dst_h / src_h
    col_scale = dst_w / src_w

    for info in point_patch_infos:
        row = info["rgb_row"] * row_scale
        col = info["rgb_col"] * col_scale
        label = str(info["label"])
        ax.scatter([col], [row], s=46, c="white", edgecolors="black", linewidths=0.8, zorder=5)
        ax.text(
            col + 2,
            row - 2,
            label,
            fontsize=8,
            color="white",
            bbox={"facecolor": "black", "alpha": 0.75, "pad": 1.5, "edgecolor": "none"},
            zorder=6,
        )
...
def _visualize(
    ...
    _annotate_point_patch_infos(
        axes[0],
        point_patch_infos,
        src_hw=(image_h, image_w),
        dst_hw=(image_h, image_w),
    )
    ...
    selected_lookup = {
        coord: {"order": idx + 1, "token": selected_patch_indices[idx]}
        for idx, coord in enumerate(selected_patch_coords_rc)
    }
    point_labels_by_patch: dict[tuple[int, int], list[int]] = {}
    for info in point_patch_infos:
        coord = (info["patch_row"], info["patch_col"])
        point_labels_by_patch.setdefault(coord, []).append(info["label"])
    for row, col in selected_patch_coords_rc:
        mask[row, col] = 1.0
    axes[2].imshow(mask, cmap="gray_r", vmin=0.0, vmax=1.0)
    axes[2].set_title("Patch Grid / Token Indices")
    ...
    for (row, col), info in selected_lookup.items():
        point_labels = point_labels_by_patch.get((row, col), [])
        point_text = ",".join(str(label) for label in point_labels) if point_labels else "-"
        axes[2].text(
            col,
            row,
            f"T{info['token']}\nP{point_text}",
    ...
    axes[3].set_title("Trajectory Point -> Token")
    axes[3].axis("off")
    mapping_lines = [
        f"P{info['label']:02d}: rgb=({info['rgb_row']},{info['rgb_col']}) -> "
        f"patch=({info['patch_row']},{info['patch_col']}) -> T{info['patch_linear_index']}"
        for info in point_patch_infos
    ]
    axes[3].text(
        0.0,
        1.0,
        "\n".join(mapping_lines) if mapping_lines else "No trajectory points labeled.",
        ha="left",
        va="top",
        fontsize=9,
        family="monospace",
...
def main() -> None:
    ...
    point_patch_infos = _build_point_patch_infos(
        pixels_rc=dense_pixels_rc,
        image_hw=(image_h, image_w),
        proc_hw=proc_hw,
        patch_hw=patch_grid_hw,
        sample_count=args.label_point_count,
    )
    ...
    _visualize(
        ...
        point_patch_infos=point_patch_infos,
    ...
    metadata = {
        ...
        "trajectory_point_patch_info": point_patch_infos,
