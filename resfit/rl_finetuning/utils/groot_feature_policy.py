...
def _map_residual_state_batch_to_groot_state(state: np.ndarray) -> np.ndarray:
    """Map residual robosuite state to the 8-D GR00T new_embodiment state."""
    state = np.asarray(state, dtype=np.float32)
    if state.ndim == 1:
        state = state[None]
    if state.ndim != 2:
    ...
    for idx, item in enumerate(state):
        if item.shape[-1] == 9:
            out[idx, :3] = item[:3]
            out[idx, 3:6] = _quat_xyzw_to_euler_xyz(item[3:7])
            out[idx, 6:8] = item[7:9]
        elif item.shape[-1] == 8:
            out[idx, :3] = item[:3]
            out[idx, 3:6] = _rotvec_to_euler_xyz(item[3:6])
            out[idx, 6:8] = item[6:8]
        else:
            raise ValueError(f"Expected residual state dim 8 or 9, got {item.shape[-1]}.")
    return out
...
class GROOTFeaturePolicy:
    def __init__(
        ...
        *,
        ...
        metadata_key = "oxe_droid" if "oxe_droid" in metadata_all else next(iter(metadata_all))
        metadata_dict = metadata_all[metadata_key]
        self._metadata = DatasetMetadata.model_validate(metadata_dict)
        self._state_stats = metadata_dict["statistics"]["state"]
        self._action_stats = metadata_dict["statistics"]["action"]
        self._video_keys = list(metadata_dict["modalities"].get("video", {}).keys())
    ...
    def _build_model_inputs(self, obs: dict[str, Any]) -> dict[str, Any]:
        ...
        if "eef_position" in self._state_stats:
            ...
            raw_state = _map_residual_state_batch_to_groot_state(obs["observation.state"])
            state = self._normalize_flat_state(raw_state)

        video_views = [base]
        if len(self._video_keys) >= 3:
            video_views.append(exterior_2)
        video_views.append(wrist)
        video = np.stack(video_views, axis=1)[:, None, ...]
    ...
    def _decode_action_chunk(self, action_tensor: torch.Tensor) -> np.ndarray:
        ...
        return self._unnormalize_flat_action(raw_actions[..., :7]).astype(np.float32)

    def _normalize_flat_state(self, state: np.ndarray) -> np.ndarray:
        if not all(key in self._state_stats for key in ("x", "y", "z", "roll", "pitch", "yaw", "gripper")):
            return state.astype(np.float32)
        return self._apply_named_min_max(
            state,
            self._state_stats,
    ...
    def _unnormalize_flat_action(self, action: np.ndarray) -> np.ndarray:
        if not all(key in self._action_stats for key in ("x", "y", "z", "roll", "pitch", "yaw", "gripper")):
            pos_delta = action[..., 0:3]
            rot_axis_angle = action[..., 3:6]
            gripper_binary = (action[..., 6:7] > 0.5).astype(np.float32)
    ...
    @staticmethod
    def _apply_named_min_max(
        values: np.ndarray,
        stats_by_key: dict[str, dict[str, list[float]]],
        keys: tuple[str, ...],
        *,
        inverse: bool,
    ) -> np.ndarray:
        ...
        for key in keys:
            ...
            min_value = np.asarray(stats["min"], dtype=np.float32)
            max_value = np.asarray(stats["max"], dtype=np.float32)
            width = int(min_value.shape[0])
            item = values[..., start : start + width]
            denom = max_value - min_value
            mask = np.abs(denom) > 1e-8
            out = np.zeros_like(item, dtype=np.float32)
            if inverse:
                out[..., mask] = ((item[..., mask] + 1.0) * 0.5) * denom[mask] + min_value[mask]
                out[..., ~mask] = min_value[~mask]
            else:
                out[..., mask] = ((item[..., mask] - min_value[mask]) / denom[mask]) * 2.0 - 1.0
                out[..., ~mask] = 0.0
            pieces.append(out)
            start += width
