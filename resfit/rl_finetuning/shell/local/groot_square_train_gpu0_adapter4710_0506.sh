...
export RESFIT_SKIP_ONLINE_CACHE_DUMP=1
...
"${PYTHON_BIN}" -u -m resfit.rl_finetuning.scripts.train_residual_td3 \
  ...
  agent.groot_feature_project_dim=128 \
  agent.groot_feature_dropout=0.1 \
  agent.groot_feature_scale_init=0.05 \
  agent.groot_feature_scale_learnable=true \
  agent.actor.spatial_emb=0 \
  agent.critic.fuse_patch=1 \
  agent.actor_lr=1e-6 \
  ...
  wandb.name=square_macro4710_grootn15_ckpt20000_off50_grootfeat_adapter128_0506_gpu0 \
  wandb.group=grootn15_macro4710_off50_grootfeat_adapter128 \
  wandb.mode=online
