...
OPENPI_REMOTE_HOST="${OPENPI_REMOTE_HOST:-127.0.0.1}"
OPENPI_REMOTE_PORT="${OPENPI_REMOTE_PORT:-8766}"

cd "${ROOT_DIR}"

"${PYTHON_BIN}" -m resfit.rl_finetuning.scripts.train_residual_td3 \
    --config-name=residual_td3_square_config \
    base_policy.provider=openpi_remote \
    base_policy.openpi_root=/data_all/gzr1/openpi \
    base_policy.openpi_remote_host="${OPENPI_REMOTE_HOST}" \
    base_policy.openpi_remote_port="${OPENPI_REMOTE_PORT}" \
    ...
    wandb.name=square_macro4710_openpi_remote_localdepth_stable \
    wandb.group=macro4710_openpi_remote_localdepth_stable \
    wandb.mode=online \
    "$@"
